...
module.exports = function (eleventyConfig) {
  ...
  // Thousands grouping with an explicit separator so EN (300,000) and VI (300.000)
  // can both read from the same number in facts.json.
  eleventyConfig.addFilter("num", (value, separator = ",") => {
  ...
  eleventyConfig.addFilter("toJsonLd", (value) =>
    JSON.stringify(value).replace(/</g, "\\u003c")
  );

  eleventyConfig.addFilter("sitemapPages", (collection, companies, leadership) => {
    const seen = new Set();
    const out = [];
    const add = (url, date) => {
    ...
    for (const page of collection || []) {
      if (!page.url || page.data?.eleventyExcludeFromCollections) continue;
      add(page.url, page.date);
    }
    for (const co of companies || []) add(`/companies/${co.id}/`);
    for (const person of leadership || []) add(`/about/leadership/${person.id}/`);
    return out;
  });
