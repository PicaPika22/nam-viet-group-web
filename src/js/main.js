...
(() => {
  ...
  const isMobileNav = () => window.matchMedia("(max-width: 1280px)").matches;
  ...
  const onHeaderScroll = () => {
    ...
    header.classList.toggle(
      "is-hidden",
      !menuOpen() && !headerHasFocus() && y > lastY && y > 500
    );
    lastY = y;
  };
  ...
  const syncNavInert = (open) => {
    if (!nav) return;
    if (isMobileNav()) nav.toggleAttribute("inert", !open);
  ...
  const setMenu = (open, { restoreFocus = true } = {}) => {
    ...
    if (isMobileNav()) {
      ...
      else if (wasOpen && restoreFocus) burger.focus();
    }
    onHeaderScroll();
  };
  if (burger && nav) {
    ...
    window.addEventListener("keydown", e => {
      ...
      const within = document.activeElement?.closest(".nav-item.has-dropdown, .nav-item.has-mega");
      if (within) {
        e.preventDefault();
    ...
    window.addEventListener("resize", () => {
      if (!isMobileNav() && menuOpen()) setMenu(false, { restoreFocus: false });
      syncNavInert(menuOpen());
  ...
  window.addEventListener("resize", syncNavPanels);
  ...
  const onScrollFx = () => {
    ...
    // Rail + header progress within page
    ...
    if (headerProgress) headerProgress.style.width = `${pct}%`;
